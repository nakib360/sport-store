...
const MyEquipmenCard = ({ card, openModal }) => {
      ...
      <div className="card bg-base-100 shadow-md rounded-md border h-full flex flex-col justify-between">
        ...
        <div className="card-body flex flex-col gap-3">
            ...
            <div className="rating">
              {[1, 2, 3, 4, 5].map((star) => (
                <input
                  ...
                  data-tooltip-content={`Rated ${rating} star`}
                />
              ))}
          ...
          <div className="mt-4 flex gap-2">
            <button
              onClick={() => navigate(`/update-equipment/${card?._id}`)}
              className="btn btn-outline hover:btn-primary w-1/2 rounded"
            >
              Update
            </button>
            <button
              onClick={() => openModal(card?._id)}
              className="btn btn-outline border-red-500 text-red-500 hover:text-black hover:bg-red-500 w-1/2 rounded"
            >
              Delete
            </button>
